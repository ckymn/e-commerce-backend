const { iyzipay, pay_form } = require("../../../../scripts/payment/iyzipay");
      ...
      if (result.status === "success") {
        ...
        if (true) {
          ...
          if (!_data)
            return next(
              new ApiError(
                "Odeme Gerceklesti ama Veri Tabanina kaydedilmedi",
                400,
                []
              )
            );
          let _register = await Register.findOneAndUpdate(
            { _id: buyer_id },
            {
              $set: {
