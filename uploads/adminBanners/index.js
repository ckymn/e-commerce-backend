...
const upload = async (file , username, bannerId) => {
  try {

    if(!file)
      return { status: 400, message: "Please upload a file!" }
    // Create a new blob in the bucket and upload the file data.
    const blob = bucket.file(username+"/"+bannerId+"/"+new Date().toISOString().replace(/:/g,"_") +"-" +file.originalname.replace(/:/g, "-"));
...
const Delete = async (username, bannerId) => {
  return  await bucket.deleteFiles({ prefix: `${username}/${bannerId}`}, (err,files) => {
       if(err)
           return err
   });
}
